...
function Count({ value, suffix }: { value: number; suffix: string }) {
  const formatted = useMemo(() => (value % 1 === 0 ? String(value) : value.toFixed(1)), [value]);
...
export function HomeSections() {
  ...
  const yFloat = useTransform(smoothY, [0, 800], [10, -10]);

  const trackMouse = (e: MouseEvent<HTMLElement>) => {
    mouseX.set(e.clientX);
    mouseY.set(e.clientY);
  };

  return (
    <main className="relative overflow-hidden" onMouseMove={trackMouse}>
      ...
      <section className="section pt-28 pb-20 md:pt-36 md:pb-24 relative">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="text-center max-w-5xl mx-auto relative z-10">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <button className="btn-primary magnetic">Join Community <ArrowRight size={16} /></button>
            <button className="btn-secondary magnetic">Explore Events</button>
          </div>
        </motion.div>

        <div className="mt-12 grid lg:grid-cols-3 gap-5 items-stretch">
          <article className="premium-card p-6 lg:col-span-2 relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-cyan-300/15 via-transparent to-brand/20" />
            <p className="text-xs tracking-[0.2em] uppercase text-cyan-300 relative">Live Community Pulse</p>
      ...
      <section className="section py-16 grid md:grid-cols-5 gap-4">
        {stats.map((s, i) => (
          <motion.article key={s.label} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ ...spring, delay: i * 0.06 }} className="premium-card stat-card p-5">
      ...
      <section className="section py-24">
        ...
        <div className="mt-10 grid lg:grid-cols-3 gap-6">
          {["Discover", "Build", "Lead"].map((t, i) => (
            <motion.article key={t} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ ...spring, delay: i * 0.1 }} className="premium-card p-7 timeline-card">
              <p className="text-sm text-cyan-300">0{i + 1}</p>
      ...
      <section className="section py-24">
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-6">
          {events.map((event) => (
            <motion.article key={event.title} whileHover={{ y: -8, scale: 1.01 }} transition={spring} className="premium-card group overflow-hidden">
      ...
      <section className="section py-24"><div className="premium-cta p-12 text-center"><h2 className="text-4xl font-bold">Join the movement shaping Bangladesh&apos;s digital careers</h2><p className="mt-4 text-slate-300">Weekly playbooks, events, mentorship, and opportunity drops.</p><button className="btn-primary magnetic mx-auto mt-8">Join Newsletter <Zap size={16} /></button></div></section>
